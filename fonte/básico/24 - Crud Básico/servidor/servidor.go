...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	alteracao, erro := statement.Exec(usuario.Nome, usuario.Email, ID)
	if erro != nil {
		fmt.Println(erro)
		w.Write([]byte("Erro ao atualizar o usuário!"))
		return
	...
	linhasAfetadas, erro := alteracao.RowsAffected()
	if erro != nil {
		w.Write([]byte("Erro ao obter quantidade de linhas afetadas!"))
		return
	}

	w.WriteHeader(http.StatusNoContent)
	w.Write([]byte(fmt.Sprintf("Usuário alterado com sucesso! Linhas afetadas: %d", linhasAfetadas)))
}
